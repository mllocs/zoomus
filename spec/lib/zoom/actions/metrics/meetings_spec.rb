...
describe Zoom::Actions::Metrics do
  ...
  let(:response) { zc.metrics_meetings(args) }
      ...
      before :each do
        stub_request(
          ...
          zoom_url('/metrics/meetings')
                    ...
                    body: json_response('metrics','meetings'),
                    headers: { 'Content-Type' => 'application/json' })
      end

      it "requires a 'from' argument" do
        expect { zc.metrics_meetings(filter_key(args, :from)) }.to raise_error(Zoom::ParameterMissing)
      end

      it "requires a 'to' argument" do
        expect { zc.metrics_meetings(filter_key(args, :to)) }.to raise_error(Zoom::ParameterMissing)
      end
